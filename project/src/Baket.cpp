...
Baket::Baket(std::string BID) {
	...
	rocksdb::Status status = rocksdb::DB::Open(options, db_path_actions, &db);
        if (!status.ok()) {
		std::cerr << "Unable to open database: " << status.ToString() << std::endl;
	}

	rocksdb::Status status_info = rocksdb::DB::Open(options, db_path_info, &db);
        if (!status_info.ok()) {
...
bool Baket::AddWordInfo(const std::string word, const std::string hash) {
	this->dbWords->Put(rocksdb::WriteOptions(), word, path);
}
